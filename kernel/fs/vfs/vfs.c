...
int vfs_fexlock_noblock(fs_handle h)
{
   struct fs_handle_base *hb = h;
   struct fs *fs = hb->fs;
   vfs_inode_ptr_t i = fs->fsops->get_inode(h);

   if (!i)
      return -EBADF;

   return vfs_exlock_noblock(fs, i);
}

int vfs_fexunlock(fs_handle h)
{
   struct fs_handle_base *hb = h;
   struct fs *fs = hb->fs;
   vfs_inode_ptr_t i = fs->fsops->get_inode(h);
