...
some typical kernel features (e.g. swap, I/O cache, SMP) introduce a substantial
amount of complexity and nondeterminism in a kernel. As mentioned above, one can
think of `Tilck` as a kernel offering what a unikernel will never be able to
offer, but without trying to be a kernel for full-blown desktop/server systems.
